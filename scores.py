...
############
def SeqToDensities(fasta,mat,sym,output,matname,matsize,name="",chrom="",th=""):
	...
	for s in sequences:
		...
		if len(chrom) != 0:
			if s.find(chrom) == -1:
				continue
		lSeq=len(seq)
		scores,nucleos = SeqToScores(seq,mat)
		i=0
		best=-70
		bestline=[]
		for sc,nuc in zip(scores,nucleos):
			i=i+1
			if len(th) != 0:
				if sc <= float(th) or sc == "NA":
					continue
			fileScores.write("%s\t%s\t%s\t+\t%s\tPWM\t%s\t%s\n"%(s,i,i+matsize,nuc,matsize,sc))
		if int(sym) == 2: # if the matrix is assymetric, look at both strands
#			print(mat)
#			print("\n")
			matRev = EvertedMatrix(mat)
			# print(matRev)
			scoresrev,nucleos = SeqToScores(seq,matRev)
			scores.extend(scoresrev)
			i=0
			for sc, nuc in zip(scoresrev,nucleos):
				i=i+1
				if len(th) != 0:
					if sc <= float(th) or sc == "NA":
						continue
			...
			if test == 1:
				...
				while n<size: 
					if (loc != []) and (n in loc): # score = dependencies score
						dep = pwmD[int(loc.index(n)/len(pwmD))]
						lDep = dep[0]
						mDep = dep[1]
						subseq = nucl[(int(lDep[0])-1):int(lDep[-1])]
...
def main(fastaFile,matFile,output,matname,chrom,th):
	if fastaFile != None or matFile != None :
		...
		if valMat == 0 :
			print("  Matrix is not conform, please reformat is following this guide :")
			error_matrix()
		else:
			print("  Matrix conformity checked successfully")
			FEAT = list(str(valMat))
			#FEAT[0] : MATRIX
			#FEAT[1] : SCORE/FREQUENCY/COUNT
			#FEAT[2] : SYMMETRIC/ASYMMETRIC
			#FEAT[3] : DEPENDENCY/SIMPLE
			matRes,matsize = modifMatrix(mat,FEAT,name)
			...
			SeqToDensities(fastaFile,matRes,FEAT[2],output,matname,matsize,name,chrom,th)
	else:
		parser.print_help()
